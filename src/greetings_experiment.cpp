...
int run_greetings(MPI_Comm comm)
{
  ...
  if (rank == 0)
  {
    ...
    for (int source = 1; source < size; ++source)
    {
      ...
      const std::string reply = "Task 0 sends greetings back from machine " + node_name
                                + " to task " + std::to_string(source);
  ...
  else
  {
    const std::string greeting =
        "Task " + std::to_string(rank) + " sends greetings from machine " + node_name;
