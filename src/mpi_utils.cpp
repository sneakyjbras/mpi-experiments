...
{
  std::vector<char> name(static_cast<std::size_t>(MPI_MAX_PROCESSOR_NAME), '\0');
  int               len = 0;
  MPI_Get_processor_name(name.data(), &len);
  return std::string(name.data(), static_cast<std::size_t>(len));
}

void send_string(const std::string& message, int destination, int tag, MPI_Comm comm)
{
...
std::string recv_string(int source, int tag, MPI_Comm comm)
{
  ...
  if (size > 0)
    MPI_Recv(message.data(), size, MPI_CHAR, source, tag + 1, comm, MPI_STATUS_IGNORE);
