...
mpi_experiments::BroadcastMetrics finalize_metrics(const std::string &label,
  ...
  const double total_bytes = static_cast<double>(payload_bytes) *
...
BroadcastMetrics benchmark_mpi_bcast_large_array(int rounds,
  ...
  return finalize_metrics("MPI_Bcast array", rounds, size, payload_bytes,
                          stop - start, comm);
}

BroadcastMetrics
benchmark_manual_broadcast_large_array(int rounds, std::size_t element_count,
                                       MPI_Comm comm) {
  if (element_count > static_cast<std::size_t>(std::numeric_limits<int>::max()))
    throw std::runtime_error(
        "Element count too large for MPI_Send / MPI_Recv int count");
  ...
  for (int round = 0; round < rounds; ++round) {
    if (rank == 0) {
      ...
      for (int destination = 1; destination < size; ++destination)
        MPI_Send(values.data(), element_count_int, MPI_DOUBLE, destination, 300,
                 comm);
    } else {
      MPI_Recv(values.data(), element_count_int, MPI_DOUBLE, 0, 300, comm,
               MPI_STATUS_IGNORE);
    }
  }
  ...
  return finalize_metrics("Manual Send/Recv", rounds, size, payload_bytes,
                          stop - start, comm);
}
