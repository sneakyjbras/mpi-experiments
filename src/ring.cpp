                                                   ...
                                                   MPI_Comm           comm)
{
  if (payload_bytes > static_cast<std::size_t>(std::numeric_limits<int>::max()))
    throw std::runtime_error("Payload too large for MPI_Send / MPI_Recv int count");
  ...
  for (int round = 0; round < rounds; ++round)
  {
    if (rank == 0)
    {
      ...
      MPI_Recv(recv_buffer.data(), byte_count, MPI_BYTE, size - 1, 0, comm, MPI_STATUS_IGNORE);
    }
    else
    {
      MPI_Recv(recv_buffer.data(), byte_count, MPI_BYTE, rank - 1, 0, comm, MPI_STATUS_IGNORE);
  ...
  metrics.latency_us_per_hop = (seconds * 1e6) / (static_cast<double>(rounds) * size);
...
{
HeatDiffusionPatch HeatDiffusionPatch::make_demo(std::size_t node_count,
                                                 std::size_t stiffness_entries)
{
  ...
  for (std::size_t i = 0; i < node_count; ++i)
    patch.nodal_temperatures[i] = patch.ambient_kelvin + 0.01 * static_cast<double>(i % 97U);
...
{
  Buffer out;
  out.reserve(sizeof(material) + 3U * sizeof(double) + 2U * sizeof(std::uint64_t)
  ...
  if (!nodal_temperatures.empty())
    append_bytes(out, nodal_temperatures.data(), nodal_temperatures.size() * sizeof(double));

  const auto stiffness_count = static_cast<std::uint64_t>(local_stiffness_matrix.size());
...
{
  const int token = 42;
  return benchmark_ring_buffer("int32 token", &token, sizeof(token), rounds, comm);
}

RingMetrics benchmark_ring_double(int rounds, MPI_Comm comm)
{
  const double temperature = 273.15;
  return benchmark_ring_buffer("double scalar", &temperature, sizeof(temperature), rounds, comm);
}
...
RingMetrics benchmark_ring_small_struct(int rounds, MPI_Comm comm)
{
  ...
  return benchmark_ring_buffer("small POD struct", &particle, sizeof(particle), rounds, comm);
}
...
RingMetrics benchmark_ring_fem_patch(int         rounds,
                                     ...
                                     MPI_Comm    comm)
{
  const auto patch   = HeatDiffusionPatch::make_demo(node_count, stiffness_entries);
  const auto payload = patch.serialize();
  return benchmark_ring_buffer("FEM heat patch", payload.data(), payload.size(), rounds, comm);
}
