...
void print_usage(const char *program) {
  std::cerr << "Usage: " << program
            << " <n-rounds> [fem-node-count] [fem-stiffness-entries]\n";
}
...
int main(int argc, char **argv) {
  ...
  results.push_back(
      mpi_experiments::benchmark_ring_int(rounds, MPI_COMM_WORLD));
  results.push_back(
      mpi_experiments::benchmark_ring_double(rounds, MPI_COMM_WORLD));
  results.push_back(
      mpi_experiments::benchmark_ring_small_struct(rounds, MPI_COMM_WORLD));
  ...
  if (rank == 0) {
    ...
    std::cout << "FEM object = HeatDiffusionPatch with " << fem_node_count
              << " nodes and " << fem_stiffness_entries
              << " stiffness entries\n\n";

    std::cout << std::left << std::setw(20) << "Payload" << std::right
              << std::setw(12) << "Bytes" << std::setw(14) << "Time (s)"
              << std::setw(20) << "Latency (us/hop)" << std::setw(22)
              << "Bandwidth (MiB/s)" << '\n';
    std::cout << std::string(88, '-') << '\n';

    for (const auto &metrics : results) {
      std::cout << std::left << std::setw(20) << metrics.label << std::right
                << std::setw(12) << metrics.payload_bytes << std::setw(14)
                << std::fixed << std::setprecision(6) << metrics.seconds
                << std::setw(20) << std::fixed << std::setprecision(3)
                << metrics.latency_us_per_hop << std::setw(22) << std::fixed
                << std::setprecision(3) << metrics.bandwidth_mib_s << '\n';
    }

    std::cout << "\nDefinitions:\n";
    std::cout << "- Latency is the average time for one message hop around the "
                 "ring.\n";
    std::cout << "- Bandwidth is the effective payload throughput:"
                 " payload_bytes * rounds * processes / total_time.\n";
    std::cout
        << "- The large FEM object is serialized first because std::vector "
           "makes the"
